...
public class ImageQuantizeMonochrome extends Component {

	/**
	 * 
	 */
	private static final long serialVersionUID = -8200679225294063715L;
	
	public ImageQuantizeMonochrome() {
	}
	...
	public int[] createGrayScalePalette(int count){
	...
	public int[] quantize(int[] pixels, int widht, int heigth, int[] colorPalette, int max_cols, boolean dither, ReductionStrategy reductionStrategy) {
	...
	public static enum ReductionStrategy {
	...
	public static void savePic(Image image, String type, String dst) {
	...
	public byte[] imageToBufferedImage(Image img) {
	...
	public Image buildImage(int width, int heigth, int pixels[]) {
		
		return createImage(new MemoryImageSource(width, heigth, pixels, 0, width));
	}
	...
	public static void main(String args[]) throws IOException, InterruptedException {
		...
		/*Image image = null;
	    try {
	    	//image = ImageIO.read(new URL("https://www.washingtonpost.com/resizer/QVsUEqQLlnCGzaHHf9PUfYzxC0A=/767x0/smart/arc-anglerfish-washpost-prod-washpost.s3.amazonaws.com/public/5VMIRBS3DYI6TGGUQRAIRUJV6I.jpg"));
	    	image = ImageIO.read(new File("D:/tao.png"));
		}catch(Exception ex) {
	    ...
	    ImageQuantizeMonochrome quantizeImage = new ImageQuantizeMonochrome();
	    
	    int[] colorPalette = quantizeImage.createGrayScalePalette(20);

	    ReductionStrategy reductionStrategy = ReductionStrategy.AVERAGE_DISTANCE;

	    int width = image.getWidth(null);
	    int heigth = image.getHeight(null);
	    int pixels[] = quantizeImage.getPixels(image);
	    quantizeImage.quantize(pixels, width, heigth, colorPalette, max_cols, dither, reductionStrategy);

	    Image imgResult = quantizeImage.buildImage(width, heigth, pixels);
	    
	    savePic(imgResult, "png", "D:/output.png");*/
		
		/*Image img = null;
	    try {
	    	//image = ImageIO.read(new URL("https://www.washingtonpost.com/resizer/QVsUEqQLlnCGzaHHf9PUfYzxC0A=/767x0/smart/arc-anglerfish-washpost-prod-washpost.s3.amazonaws.com/public/5VMIRBS3DYI6TGGUQRAIRUJV6I.jpg"));
	    	img = ImageIO.read(new File("D:/ElcomTag213C3BWR_FB6DEC499_VERSION1_TAG9_1576045688.png"));
		}catch(Exception ex) {
			System.out.println("ex: " + ex.toString());
		}*/
		
		loadFonts(new String[]{"unifont-12.1.04.ttf"
				});
		String fontName = "Unifont";
		
		BufferedImage image = new BufferedImage(212, 104, BufferedImage.TYPE_INT_RGB);
		
		Graphics2D g = image.createGraphics();
		
		g.setColor(Color.WHITE);
		g.fillRect(0, 0, 212, 104);
		
		g.setColor(Color.RED);
		g.fillRect(0, 0, 212, 37);
		
		g.setColor(Color.WHITE);
	    g.setFont(new Font(fontName, Font.PLAIN, 18));
	    g.drawString("ÔỐỒỔỖỘôốồổỗộâấầẩẫậ", 2, 22);
		
		g.setColor(Color.WHITE);
		g.fillRect(0, 40, 140, 30);
		
	    g.setColor(Color.BLACK);
	    //g.setFont(new Font("Roboto", Font.PLAIN, 10));
	    
	    AttributedString as = new AttributedString("đỗ việt anh 1");
	    as.addAttribute(TextAttribute.FONT, new Font(fontName, Font.PLAIN, 15));
	    g.drawString(as.getIterator(), 2, 55);
	    
	    as = new AttributedString("đỗ việt anh 2");
	    as.addAttribute(TextAttribute.FONT, new Font(fontName, Font.BOLD, 15));
	    g.drawString(as.getIterator(), 2, 75);
	    
	    g.setFont(new Font(fontName, Font.PLAIN, 17));
	    g.drawString("4,690,000 vnđ Đ", 2, 95);
	    
	    g.setColor(Color.RED);
	    g.fillOval(140, 40, 75, 48);
	    
	    g.setColor(Color.WHITE);
	    g.setFont(new Font(fontName, Font.BOLD, 14));
	    g.drawString("Bớt 25 %", 150, 70);
	    
	    g.dispose();
	    
	    ImageIO.write(image, "png", new File("D:/output-final.png"));
	    
	    System.exit(0);
	}
	
	private static void loadFonts(String[] fontArr) {
		...
		try {
			...
			for( String font : fontArr ) {
				...
				ge.registerFont(Font.createFont(Font.TRUETYPE_FONT, new FileInputStream(new File("D:\\SetUp\\apache-tomcat-8.5.45\\webapps\\elcomService\\WEB-INF\\resources\\fonts\\" + font))));
			}
		} catch(Exception ex) {
			System.out.println("loadFont(): ERROR: " + ex.toString());
		}
	}
}
